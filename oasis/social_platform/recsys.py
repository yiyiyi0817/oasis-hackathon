...
def get_trace_contents(user_id, action, post_table, trace_table):
    ...
    # Get post IDs from trace table for the given user and action
    trace_post_ids = [
        trace['info']['post_id'] for trace in trace_table
        if (trace['user_id'] == user_id and trace['action'] == action and trace['info']['post_id'])
    ]
            ...
            if swap_rate > 0:
                ...
                swap_free_ids = [
                    ...
                    if post_id not in rec_post_ids and post_id not in [
                        trace.get('post_id', None)
                        for trace in trace_table if trace['user_id'] and trace.get('post_id') is not None
                    ]
                ]
